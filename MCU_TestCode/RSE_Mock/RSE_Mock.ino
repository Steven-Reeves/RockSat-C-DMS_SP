 ...
 * Modified:    6/3/18
 * Filename:    RSE_Mock.ino
 * 
 * Note: This code is commented out to return mock data for testing and presentation.
 * 
...
void loop() {
  ...
  //Serial.println("Setting fake hits");
  tubeOneHits = random(0,2);
  tubeTwoHits = random(0,2);
  tubeThreeHits = random(0,6);
  tubeFourHits = random(0,9);
  if (tubeOneHits > 0)
  {
...
void InitSd()
{
  ...
  while (!SD.begin(chipSelect) && i++ < 100) 
    { 
      delay(10);
      Serial.println("Tried to open SD card i = " + i); 
    } // Try to initialize SD card for up to 1 second.
